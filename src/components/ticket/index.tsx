...
const Ticket: React.FC = () => {
  ...
  const { sortedTickets } = useSelector((state: RootState) => state.sorted);
  ...
  return (
    <>
      {sortedTickets.length === 0 ? (
        <article style={{ marginTop: '20px', color: 'red' }}>
          <Alert
            message="Выберите фильтры для поиска"
            description="Для отображения билетов установите параметры фильтрации, которые соответствуют вашим предпочтениям"
            type="info"
            showIcon
          />
        </article>
